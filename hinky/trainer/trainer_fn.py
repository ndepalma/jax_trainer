...
def _eval_model(
  ...
  *,
  ...
  eval_metrics = eval_stepper.init_eval_metrics(
    ...
    batch=trainer.batch_to_input(val_loader[0]),
    rngs=rngs,
  )
  step_count = 0
...
def train_model(
  ...
  if ds_tables.val:
    ...
    eval_step.test_eval_function(trainer, val_tbl.to_batches(max_chunksize=trainer.dataset_config.training_params.batch_size), rngs=trainer.rngs_eval)
  all_eval_metrics = {}
  train_metrics = None
  ...
  for epoch_idx in trainer.tracker(progress_table, iter(range(1, num_epochs + 1)), desc=""):
    ...
    train_tbl = ds_tables.train
    train_metrics, epoch_metrics = epoch_trainer(
      progress_table,
      train_tbl.to_batches(max_chunksize=trainer.dataset_config.training_params.batch_size),
      epoch_idx=epoch_idx,
    ...
    if run_validation and ds_tables.val is not None:
      ...
      eval_metrics = _eval_model(
        ...
        trainer=trainer,
        val_loader=ds_tables.val.to_batches(max_chunksize=trainer.dataset_config.training_params.batch_size),
        mode="val",
        epoch_idx=epoch_idx,
