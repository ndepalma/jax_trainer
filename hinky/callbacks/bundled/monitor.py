...
class GradientSpikeMonitor(Callback):
  ...
  def __init__(
    ...
    self.max_elements = int(math.log(1e-3) / math.log(self.ema_decay))
    self.trainer = trainer

  def on_training_start(self) -> None:
    self.grad_norms_buffer = []
    self.losses_buffer = []
    self.grad_norms = jnp.array([], dtype=jnp.float64)
    self.losses = jnp.array([], dtype=jnp.float64)
  ...
  def on_filtered_training_epoch_end(self, train_metrics: dict[str, Any], epoch_idx: int) -> None:
    del train_metrics
    epoch_grad_norms = jnp.asarray(jax.device_get(self.grad_norms_buffer))
    epoch_losses = jnp.asarray(jax.device_get(self.losses_buffer))
    self.grad_norms = jnp.concatenate([self.grad_norms, epoch_grad_norms])
    self.losses = jnp.concatenate([self.losses, epoch_losses])
    self.grad_norms_buffer = []
    self.losses_buffer = []
    if self.log_to_disk:
      np.savez(
  ...
  def log_gradients_spikes(self, num_elements: int, epoch_idx: int) -> None:
    ...
    weights = jnp.zeros_like(grad_norms) + self.ema_decay
    weights = jnp.power(weights, jnp.flip(jnp.arange(grad_norms.shape[0])))
    # Normalize all means with respect to all previous weights.
    weight_cumsum = jnp.cumsum(weights)
    grad_norms_cumsum = jnp.cumsum(grad_norms * weights)
    losses_cumsum = jnp.cumsum(losses * weights)
    ...
    grad_norms_spike = grad_norms_spike & jnp.logical_not(
      jnp.concatenate([jnp.array([False]), grad_norms_spike[:-1]]),
    )
    losses_spike = losses_spike & jnp.logical_not(
      jnp.concatenate([jnp.array([False]), losses_spike[:-1]]),
    )
    # Log the spikes.
    grad_norms_num_spikes = jnp.sum(grad_norms_spike)
    losses_num_spikes = jnp.sum(losses_spike)
    ...
    synchronous_spikes = jnp.sum(grad_norms_spike & losses_spike)
    self.trainer.logger.log_scalar("optimizer/spikes_synchronous", synchronous_spikes, epoch_idx)
    grad_spike_before_loss = jnp.sum(
      grad_norms_spike & jnp.concatenate([jnp.array([False]), losses_spike[:-1]]),
    )
    self.trainer.logger.log_scalar(
      "optimizer/spikes_grad_before_loss", grad_spike_before_loss, epoch_idx,
    )
    loss_spike_before_grad = jnp.sum(
      losses_spike & jnp.concatenate([jnp.array([False]), grad_norms_spike[:-1]]),
    )
    self.trainer.logger.log_scalar(
